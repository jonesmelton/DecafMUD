...
(function (DecafMUD) {
  ...
  FlashSocket.prototype.setup = function (count) {
    ...
    if (window.swfobject === undefined) {
      ...
      if (count < 100) {
        this.decaf.timer = setTimeout(function () {
          sock.setup(count + 1);
        }, 25);
        return;
      } else {
        this.decaf.error(
          "Unable to load SWFObject. Please download it from {0}.".tr(
    ...
    div.style.cssText =
      "display:none;position:absolute;width:0;height:0;left:-9000px;overflow:hidden!important;margin:0!important;padding:0!important;";
    div.id = this.id;
  ...
  FlashSocket.prototype.noSocket = function () {
    clearTimeout(this.decaf.timer);
    this.decaf.error(
      "Unable to create a Flash socket. Please ensure that you have the Adobe Flash plugin installed and that it's currently up to date."
    );
  };
  ...
  FlashSocket.prototype.onSocket = function (e) {
    ...
    if (e.success) {
      ...
      if (!swfobject.hasFlashPlayerVersion("9.0.0")) {
        ...
        if (ver.major === 0) {
          this.decaf.error(
            'DecafMUD\'s Flash connectivity requires Adobe Flash 9 or later. Please <a href="http://get.adobe.com/flashplayer/">install</a> Flash.'
          );
        } else {
          this.decaf.error(
            'DecafMUD\'s Flash connectivity requires at least Adobe Flash version 9.0. You have version {0}.{1}.{2}. Please <a href="http://get.adobe.com/flashplayer/">upgrade</a> your Flash plugin.'.tr(
              ver.major,
              ver.minor,
              ver.release
            )
        ...
        this.decaf.error(
          "There was an unknown error (though most likely a 404) loading the Flash socket file."
        );
      }
    }
  };
  ...
  FlashSocket.prototype.connect = function () {
    ...
    if (
      this.decaf.options.set_socket.policyport !== 843 &&
      this.decaf.options.set_socket.policyport !== undefined
    ) {
  ...
  FlashSocket.executeCallback = function (id, type, data, data2) {
    ...
    if (type === 0) {
      ...
      sock.decaf.debugString(
        'Unknown Flash Callback: type={0} data="{1} data2="{2}"'.tr(
          type,
          data,
          data2
        )
