...
function start_zmp() {
  decaf.zmp.addFunction("char.vitals", function (cmd, val) {
    handle_variables(val);
  });
  decaf.zmp.addFunction("Char.Vitals", function (cmd, val) {
    handle_variables(val);
  });
  decaf.zmp.addFunction("room.map", function (cmd, val) {
    show_map(val);
  });
  decaf.zmp.addFunction("lpc.edit", function (cmd, val) {
    init_edit(val);
  });
  decaf.zmp.addFunction("lpc.file", function (cmd, val) {
    edit_file(val);
  });
  decaf.zmp.addFunction("lpc.written", function (path) {
    saved_file(path);
  });
}
...
function update_bars() {
  if (!progresscreated) {
    ...
    lli.innerHTML =
      '<a href="javascript:menu_progressbars()">' + "Show/Hide Bars</a>";
    opt.appendChild(lli);
  }
  ...
  if (gp != -1 && maxgp != -1)
    decaf.ui.setProgress(
      "Gp",
      (gp * 100) / maxgp,
      "" + gp + " (" + maxgp + ")"
    );
  if (bdn != -1) decaf.ui.setProgress("Bdn", bdn, "" + bdn + "%");
  xpspan.innerHTML = xp;
}
...
function show_map(value) {
  ...
  if (!showmap) {
    ...
    lli.innerHTML = '<a href="javascript:menu_map()">' + "Hide Map</a>";
    opt.appendChild(lli);
  }

  mmap = htmlify(mmap);

  decaf.ui.mapdiv.innerHTML = "<hr><i>Map:</i><center>" + mmap + "</center>";
}
