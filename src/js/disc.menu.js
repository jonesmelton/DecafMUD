...
var toolbar_menus = [
  ...
  [
    ...
    ["HTML log", "menu_log('html');", "Plain Text Log", "menu_log('plain');"],
  ],
  [
    "Options",
    "menu_options",
    "Change DecafMud Options",
    [
      "Triggers",
      "menu_triggers();",
      "Macros",
  ...
  [
    "Help",
    "menu_help",
    "Get help with Discworld and Client problems.",
    [
      "Troubleshooting",
      "menu_trouble();",
      "Rules",
...
function build_menu(id) {
  ...
  for (let j = 0; j < toolbar_menus[id][3].length; j += 2) {
    ret +=
      ...
      toolbar_menus[id][3][j + 1] +
      '">' +
      toolbar_menus[id][3][j] +
      "</a></li>";
  }
  ret += "</ul>";
  return ret;
...
function menu_log(style) {
  ...
  // get the log file
  var txt = document
    .getElementById("mud-display")
    .innerHTML.substr(start_content);
  if (style == "plain") {
    txt = txt.replace(/\n/g, " ");
  ...
  add_element(
    popup,
    "p",
    "To log, copy the text from this area to " +
...
function menu_font_size() {
  ...
  add_element(
    frm,
    "p",
    "(Select a value between 50 and 500 - the " + "default size is 100.)"
  );
  add_element(
    frm,
    "p",
  ...
  add_element(
    frm,
    "p",
    "(Select a font that is supported by your " +
...
function change_font() {
  ...
  if (s != "")
    decaf.ui.el_display.style.fontFamily =
      "'" +
      s +
      "', Consolas, " +
...
function menu_macros() {
  ...
  add_element(
    pop,
    "p",
    "Decafmud supports both F-key macro's " +
  ...
  add_element(
    frm,
    "p",
    '<input type="checkbox" name="cfkey" ' +
      (fkeymacros ? "checked" : "") +
      "/>Enable f-key macros."
  );
  add_element(
    frm,
    "p",
    '<input type="checkbox" name="cnumpad" ' +
      (numpadwalking ? "checked" : "") +
      "/>Enable numpad navigation."
  );
  var savebtn = document.createElement("a");
  savebtn.className = "fakebutton";
...
function menu_map() {
  ...
  if (showmap == 1) {
    toggle_map(false);
    c.innerHTML = '<a href="javascript:menu_map()">' + "Show Map</a>";
  } else {
    toggle_map(true);
    c.innerHTML = '<a href="javascript:menu_map()">' + "Hide Map</a>";
  }
}

function menu_rules() {
  window.open(
    "http://discworld.atuin.net/lpc/playing/documentation.c?path=/concepts/rules",
    "Rules",
    "width=800,height=400,resizable=yes,scrollbar=yes,toolbar=yes,menubar=no,location=no,directories=no,status=no"
  );
}
...
function menu_features() {
  ...
  add_element(
    pop,
    "p",
    "Decafmud is a basic mud client tailored " +
  ...
  add_element(
    el,
    "li",
    "To send multiple commands at once, separate " +
  ...
  add_element(
    el,
    "li",
    "You can browse your previous commands with " +
      "the up and down arrow keys."
  );
  add_element(
    el,
    "li",
    "The F1, F2, ... keys send the commands f1, " +
  ...
  add_element(
    el,
    "li",
    "You can use the numpad for quick " +
  ...
  add_element(
    el,
    "li",
    "To create a log file from your current " +
  ...
  add_element(
    pop,
    "p",
    "If you want more features, or DecafMud " +
...
function build_trigger_list() {
  ...
  for (i = triggers.length - 1; i >= 0; i--) {
    t +=
      ...
      '">' +
      triggers[i][0] +
      '</span> - <a href="javascript:remove_trigger(' +
      "'" +
      triggers[i][0].replace(/[\'']/g, "\\'") +
      "')\">remove</a></li>";
  }
  return t;
}
...
function build_sound_selector() {
  ...
  for (i = 0; i < t_sounds.length; i++) {
    if (i == 0) {
      s +=
        '<option value="' +
        t_sounds[i].toLowerCase().replace(" ", "_") +
        '" selected>' +
        t_sounds[i] +
        "</option>";
    } else {
...
function add_trigger() {
  ...
  var add_error = DecafMUD.plugins.Display.standard.prototype.addTrigger(
    phrase.value,
    color,
    sound
  );
...
function menu_triggers() {
  ...
  add_element(
    frm,
    "span",
    '<input class="breathingroom" id="wordentry" type="text" size=20 placeholder="Enter Word or Phrase" value="">' +
      build_color_selector() +
      build_sound_selector() +
      "<br><br>"
  );
  ...
  add_element(
    frm,
    "span",
    '<ul id="live_triggers">' + build_trigger_list() + "</ul>"
  );
}
