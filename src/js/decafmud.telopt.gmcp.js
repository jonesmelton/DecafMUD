...
(function (DecafMUD) {
  ...
  GMCP.prototype._sb = function (data) {
    ...
    var ind = data.search(/[^A-Za-z0-9._]/),
      ret = false,
      pckg,
      out;
    if (ind !== -1) {
...
/** The package structure. */
  ...
  GMCP.prototype.packages.Core = {
    ...
    Ping: function (data) {
      ...
      this.pingCount++;
      this.pingAverage = Math.ceil(
        (n + this.pingAverage * (this.pingCount - 1)) / this.pingCount
      );
      console.debug(
        "PING: {0}ms over {1} pings".tr(this.pingAverage, this.pingCount)
      );
    },
