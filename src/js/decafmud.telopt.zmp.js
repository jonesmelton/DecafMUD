...
(function (DecafMUD) {
  ...
  ZMP.prototype._sb = function (data) {
    ...
    this.decaf.debugString(
      "RCVD " + DecafMUD.debugIAC(t.IAC + t.SB + t.ZMP + data + t.IAC + t.SE)
    );

    //        alert("Getting zmp data!");
    //        alert("cmd = " + cmd);
    //        alert("dat = " + dat);
      ...
      for (var i = 0, l = data.length; i < l; i++) {
        ...
        if (c.length > 0) {
          func = this.getFunction(
            c.substr(-1) == "." ? c.substr(0, c.length - 1) : c,
            true
          );
          if (func === undefined) {
