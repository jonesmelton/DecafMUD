...
function dw_interface_wrapper(DecafMUD) {
  ...
  var SimpleInterface = function (decaf) {
    ...
    addEvent(this.el_display, "keydown", function (e) {
      si.displayKey(e);
    });
    addEvent(this.sidebar, "keydown", function (e) {
      si.displayKey(e);
    });
  ...
  SimpleInterface.prototype.initSplash = function (percentage, message) {
    ...
    if (message === undefined) {
      message = "Discombobulating interface recipient...";
    }
    ...
    this.splash.innerHTML =
      '<h2 class="decafmud heading">DecafMUD <span class="version">v' +
      DecafMUD.version +
      "</span></h2>";
  ...
  SimpleInterface.prototype.updateSplash = function (percentage, message) {
    if (this.splash === null || this.splash_err) {
      return;
    }
    if (percentage !== undefined) {
  ...
  SimpleInterface.prototype.showSize = function () {
    ...
    var si = this;
    this.sizetm = setTimeout(function () {
      si.hideSize();
    }, 500);
  };
  ...
  SimpleInterface.prototype.hideSize = function (fnl) {
    ...
    if (fnl === true) {
      ...
      if (this.decaf.telopt[DecafMUD.TN.NAWS] !== undefined) {
        try {
          this.decaf.telopt[DecafMUD.TN.NAWS].send();
        } catch (err) {}
      }
  ...
  SimpleInterface.prototype.connected = function () {
    this.updateIcon(
      this.ico_connected,
      "DecafMUD is currently connected.",
      "",
  ...
  SimpleInterface.prototype.connecting = function () {
    ...
    if (this.decaf.options.socket == "websocket") {
      this.display.message(
        "<span>You are connecting using <i>websockets</i> " +
          "on port " +
          this.decaf.options.set_socket.wsport +
          ".  If this does " +
      ...
      this.display.message(
        "<span>You are connecting using <i>flash</i> " +
          "on port " +
          this.decaf.options.port +
          ".  To connect using " +
  ...
  SimpleInterface.prototype.setup = function () {
    ...
    this.decaf.debugString(
      'Initializing display plugin "' +
        display +
        '" in: #' +
        this.el_display.id,
      "info"
    );
    this.display = new DecafMUD.plugins.Display[display](
      this.decaf,
      this,
      this.el_display
    );
    ...
    this.ico_connected = this.addIcon(
  ...
  SimpleInterface.prototype.tbDelete = function (id) {
    if (this.toolbuttons[id] === undefined) {
      return;
    }
  ...
  SimpleInterface.prototype.tbText = function (id, text) {
    ...
    if (!text) {
      throw "Text can't be empty/false/null/whatever.";
    }
  ...
  SimpleInterface.prototype.tbPressed = function (id, pressed) {
    ...
    if (pressed) {
      if (/toggle-depressed/.test(btn[0].className)) {
        btn[0].className = btn[0].className.replace(
          " toggle-depressed",
          " toggle-pressed"
        );
      }
  ...
  SimpleInterface.prototype.tbClass = function (id, clss) {
    ...
    var old_clss = btn[7];
    btn[7] = clss;
    if (old_clss !== undefined) {
      btn[0].className = btn[0].className.replace(" " + old_clss, "");
    }
    if (clss) {
      btn[0].className += " " + clss;
    }
  };
  ...
  SimpleInterface.prototype.tbIcon = function (id, icon) {
    ...
    btn[2] = icon;
    if (icon) {
      if (!/ icon/.test(btn[0].className)) {
        btn[0].className += " icon";
      }
  ...
  SimpleInterface.prototype.tbNew = function (
    ...
    if (type === 1) {
      btn.className +=
        " toggle " + (pressed ? "toggle-pressed" : "toggle-depressed");
    }
    btn.innerHTML = text;
    if (tooltip) {
      btn.title = tooltip;
  ...
  SimpleInterface.prototype.createIBar = function () {
    ...
    var closer = function (e) {
      if (
        e === undefined ||
        (e.type === "keydown" && e.keyCode !== 13 && e.keyCode !== 27)
      ) {
        return;
      }
      if (e.type === "click" && !ibar.click) {
        return;
      }
    ...
    var helper = function (e) {
      if (e === undefined || (e.type === "keydown" && e.keyCode !== 13)) {
        return;
      }
      si.closeIBar(true);
      if (ibar.close) {
    ...
    setTimeout(function () {
      ...
      if (si.toolbarPadding) {
        pt += si.toolbarPadding - 10;
      }
      obj.style.cssText =
  ...
  SimpleInterface.prototype.addIcon = function (
    ...
    ico.className =
      "decafmud status-icon " + clss + (onclick ? " icon-click" : "");
    ico.innerHTML = html;
    ...
    for (var i = 0; i < this.icons.length; i++) {
      this.icons[i][0].style.cssText =
        "right:" + (this.icons.length - i - 1) * 21 + "px";
    }
    ...
    var si = this;
    if (onclick) {
      addEvent(ico, "click", function (e) {
        onclick.call(si, e);
      });
    }
    if (onclick && !onkey) {
      addEvent(ico, "keydown", function (e) {
        if (e.keyCode !== 13) {
          return;
        }
        onclick.call(si, e);
  ...
  SimpleInterface.prototype.resizeScreen = function (showSize, force) {
    ...
    if (this.toolbarPadding) {
      tot = tot - (this.toolbarPadding - 12);
    }
    if (tot < 0) {
      tot = 0;
    }
    ...
    if (this.popup) {
      this.set_mid.style.height = tot + "px";
    }
    if (this.toolbarPadding) {
      tot -= 12;
      if (tot < 0) {
    ...
    if (this.scrollButton) {
      this.scrollButton.style.cssText =
        "bottom:" + (this._input.offsetHeight + 12) + "px";
    }
  ...
  SimpleInterface.prototype.updateInput = function (force) {
    ...
    if (!this.echo) {
      ...
      if (cl) {
        new_inp.className = cl;
      }
      if (st) {
        new_inp.setAttribute("style", st);
      }
      ...
      if (pos) {
        par.insertBefore(new_inp, pos);
      } else {
        par.appendChild(new_inp);
      }
      this.input = new_inp;
      this.input.value = buffer;

      // Attach an event listener for onKeyDown.
      addEvent(new_inp, "keydown", function (e) {
        si.handleInputPassword(e);
      });
    } else {
      // Not a password.
      var lines = 1,
