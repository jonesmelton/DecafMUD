...
if (String.prototype.endsWith === undefined) {
  ...
  String.prototype.endsWith = function (suffix) {
    ...
    return startPos < 0
      ? false
      : this.lastIndexOf(suffix, startPos) === startPos;
  };
}
...
(function (window) {
  ...
  // Default Values
  DecafMUD.prototype.debug =
    DEBUG_MODE; /** NOTE: THIS IS THE MASTER SWITCH FOR DEV DEBUGGING */
 ...
 * @namespace All the available plugins for {@link DecafMUD}, in one easy-to-access
  ...
  DecafMUD.plugins.Encoding.utf8 = {
    ...
    decode: function (data) {
      ...
      try {
        ...
        console.log("unicode according to this guy", [out, data.substr(i)]);
  ...
  var tTTYPE = function (decaf) {
    this.decaf = decaf;
  };
  tTTYPE.prototype.current = -1;
  tTTYPE.prototype._dont = tTTYPE.prototype.disconnect = function () {
    this.current = -1;
  };
  tTTYPE.prototype._sb = function (data) {
    ...
    this.decaf.debugString(
      "RCVD " +
        DecafMUD.debugIAC(t.IAC + t.SB + t.TTYPE + t.ECHO + t.IAC + t.SE)
    );
    this.decaf.sendIAC(
      t.IAC +
        t.SB +
        t.TTYPE +
        t.IS +
        this.decaf.options.ttypes[this.current] +
        t.IAC +
        t.SE
    );
  ...
  tNAWS.prototype._dont = tNAWS.prototype.disconnect = function () {
    this.enabled = false;
  };
  ...
  tCHARSET.prototype._will = function () {
    ...
    setTimeout(function () {
      ...
      if (
        e !== "iso88591" &&
        DecafMUD.plugins.Encoding[e] !== undefined &&
        DecafMUD.plugins.Encoding[e].proper !== undefined
      ) {
      ...
      for (var k in DecafMUD.plugins.Encoding) {
        if (
          ...
          DecafMUD.plugins.Encoding[k].proper === undefined
        ) {
          continue;
        }
      ...
      c.decaf.sendIAC(
        t.IAC + t.SB + t.CHARSET + t.ECHO + " " + cs.join(" ") + t.IAC + t.SE
      );
    }, 0);
  };
  tCHARSET.prototype._sb = function (data) {
    ...
    this.decaf.debugString(
      "RCVD " +
        DecafMUD.debugIAC(t.IAC + t.SB + t.CHARSET + data + t.IAC + t.SE)
    );
    ...
    if (data.charCodeAt(0) === 1) {
      ...
      if (e !== undefined) {
        ...
        this.decaf.sendIAC(
          t.IAC + t.SB + t.CHARSET + "\x02" + o + t.IAC + t.SE
        );
      } else {
        // Reject it.
    ...
    while (data.length > 0) {
      ...
      if (c === 2) {
        ...
        if (out[variable] === undefined) {
          ...
          typeof out[variable] === "object" &&
          out[variable].push !== undefined
        ) {
  ...
  tMSDP.prototype._will = function () {
    ...
    setTimeout(function () {
      m.decaf.sendIAC(
        t.IAC + t.SB + t.MSDP + "\x01LIST\x02COMMANDS" + t.IAC + t.SE
      );
      m.decaf.sendIAC(
        t.IAC + t.SB + t.MSDP + "\x01LIST\x02VARIABLES" + t.IAC + t.SE
      );
      m.decaf.sendIAC(
        t.IAC +
          t.SB +
          t.MSDP +
          "\x01LIST\x02CONFIGURABLE_VARIABLES" +
          t.IAC +
          t.SE
      );
      m.decaf.sendIAC(
        t.IAC +
          t.SB +
          t.MSDP +
          "\x01LIST\x02REPORTABLE_VARIABLES" +
          t.IAC +
          t.SE
      );
    }, 0);
  ...
  tMSDP.prototype._sb = function (data) {
    var out = readMSDP(data)[0],
      ret = false; // We don't care about the left over string.

    // Debug it before we do anything potentially destructive.
    if ("console" in window && console.groupCollapsed) {
      console.groupCollapsed(
        "DecafMUD[" + this.decaf.id + "]: RCVD IAC SB MSDP ... IAC SE"
      );
      console.dir(out);
      console.groupEnd(
        "DecafMUD[" + this.decaf.id + "]: RCVD IAC SB MSDP ... IAC SE"
  ...
  DecafMUD.prototype.debugString = function (text, type, obj) {
    ...
    switch (type) {
      case "info":
        console.info(st, this.id, text);
        return;
      case "warn":
        console.warn(st, this.id, text);
        return;
      case "error":
        console.error(st, this.id, text);
        return;
      default:
  ...
  DecafMUD.prototype.error = function (text) {
    ...
    if (this.ui && this.ui.splashError(text)) {
      return;
    }
  ...
  DecafMUD.prototype.noFlash = function () {
    answer = confirm(
      "Could not establish a flash connection.  This may be " +
        "because you do not have flash installed, or because your browser is " +
        "blocking it.\n\n" +
        "A beta version of this client uses websockets instead.  Would you like " +
        "to try this out?\n"
    );
    if (answer) {
      window.location = "web_client.html";
      return 1;
    }
    return 0;
  };
  ...
  DecafMUD.prototype.loadScript = function (filename, path) {
    if (path === undefined) {
      if (this.options.jslocation !== undefined) {
        path = this.options.jslocation;
      }
      if (
        path === undefined ||
  ...
  DecafMUD.prototype.waitLoad = function (next, itemloaded, tr) {
    ...
    if (tr === undefined) {
      ...
      if (this.need[0][0].indexOf("language") === -1) {
        this.error(
          "Timed out attempting to load the module: {0}".tr(this.need[0][0])
        );
        return;
      } else {
    ...
    if (this.need.length === 0) {
      ...
      this.loadTimer = setTimeout(function () {
        decaf.waitLoad(next, itemloaded, tr + 1);
      }, this.options.wait_delay);
    }
  };
  ...
  DecafMUD.prototype.initSocket = function () {
    ...
    this.debugString(
      'Creating a socket using the "{0}" plugin.'.tr(this.options.socket)
    );
  ...
  DecafMUD.prototype.initFinal = function () {
    ...
    if (/MSIE/.test(navigator.userAgent) && this.ui.infoBar) {
      var msg =
        "You may experience poor performance and UI glitches using " +
    ...
    this.ui.display.message(
      "If this program is failing to connect, " +
        "check whether you have a flash blocker and if so, turn it off " +
        'for Discworld.  Otherwise, connect using <a href="web_client.html">' +
        "websockets</a>, check the Help -> Troubleshooting menu or click " +
        '<a href="help.html#connection" target="new">here</a> for more ' +
        "information.<br/>"
    );
  ...
  DecafMUD.prototype.connect = function () {
    ...
    if (this.socket_ready !== true) {
      throw "The socket isn't ready yet.";
    }
    ...
    var decaf = this;
    this.conn_timer = setTimeout(function () {
      decaf.connectFail();
  ...
  DecafMUD.prototype.socketConnected = function () {
    ...
    this.debugString(
      "The socket has connected successfully to {0}:{1}.".tr(host, port),
      "info"
    );
      ...
      if (this.connect_try < this.options.reconnect_tries) {
        ...
        if (this.ui && this.ui.immediateInfoBar && s >= 0.25) {
          this.ui.immediateInfoBar(
            "You have been disconnected. Reconnecting in {0} second{1}...".tr(
              s,
              s === 1 ? "" : "s"
            ),
            "reconnecting",
  ...
  DecafMUD.prototype.readIAC = function (data) {
    ...
    // If the second character is a GA, ignore it.
    else if (data.charCodeAt(1) === 249) {
      return data.substr(2);
    }
    ...
    // If it's an IAC SB, read as much as we can to get it all.
      ...
      while (data.length > 0) {
        ...
        if (ind === -1) {
          ...
          /*
                                ...
                                // if a sequence this long is still
                                // unterminated, it's not going to be
                                return "ERROR: unterminated IAC SB\n" +
                                       this.readIAC(data);
                                */
        }
      ...
      if (
        this.telopt[code] !== undefined &&
        this.telopt[code]._sb !== undefined
      ) {
        if (this.telopt[code]._sb(seq) === false) {
          dbg = false;
        }
      }
      ...
      if (dbg) {
        if (code === t.MSSP && console.groupCollapsed !== undefined) {
          console.groupCollapsed(
            "DecafMUD[" + this.id + "]: RCVD IAC SB MSSP ... IAC SE"
          );
          ...
          this.debugString(
            "RCVD " +
              DecafMUD.debugIAC(t.IAC + t.SB + code + seq + t.IAC + t.SE)
          );
        }
      }
    }
