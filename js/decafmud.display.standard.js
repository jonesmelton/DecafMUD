...
(function (DecafMUD) {
  ...
  var Display = function (decaf, ui, disp) {
    ...
    this.display.className =
      "decafmud display " + this.decaf.options.set_display.fgclass + "7";
    this._display.appendChild(this.display);

    // Attach the scroll event.
    var d = this;
    addEvent(this._display, "scroll", function (e) {
      d.onScroll();
    });
    ...
    addEvent(this._display, "mousedown", function (e) {
      ...
      if (
        e.which !== 2 ||
        !d.decaf.store.get("ui/middle-click-scroll", false)
      ) {
        return;
      }
      d.scroll();
  ...
  Display.prototype.looks_like_html = function (s) {
    return (
      ["NBSP", "SPA", "SPAN", "ASS", "LASS", "CLASS", "HTML"].indexOf(s) > -1
    );
  };
...
essentially just leave the string alone. */
  ...
  Display.prototype.playTriggerSound = function (soundName) {
    ...
    if (!(soundName in self.triggerSounds)) {
      self.triggerSounds[soundName] = new Audio(
 ...
 @param {String} data The data to be displayed. */
  ...
  Display.prototype.shouldScroll = function (addTarget) {
    if (
      ...
      this._display.style.overflowY === "hidden"
    ) {
      return;
    }
    this.willScroll =
  ...
  Display.prototype.tags = {
    ...
    COLOR: {
      ...
      want: false,
      open_tag:
        '<span class="mxp mxp-color" style="color:&fore;;background-color:&back;">',
      close_tag: "</span>",
      arg_order: ["fore", "back"],
      arguments: {
        fore: "inherit",
